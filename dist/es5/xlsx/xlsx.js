...
var CommentXform = require('./xform/comment/comments-xform');
...
XLSX.prototype = {
  ...
  createInputStream: function createInputStream(options) {
    ...
    stream.on('entry', function (entry) {
      ...
      switch (entryPath) {
        ...
        default:
          promise = self.processWorksheetEntry(entry, model, options) || self.processWorksheetRelsEntry(entry, model) || self.processThemeEntry(entry, model) || self.processMediaEntry(entry, model) || self.processDrawingEntry(entry, model) || self.processCommentEntry(entry, model) || self.processDrawingRelsEntry(entry, model) || self.processIgnoreEntry(entry);
          break;
      }
  ...
  addWorksheets: function addWorksheets(zip, model) {
    return new PromiseLib.Promise(function (resolve) {
      ...
      var commentXform = new CommentXform(); // write sheets
